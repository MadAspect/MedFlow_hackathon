...
function refresh() {
  ...
  loadPage().then(function (html) {
    ...
    if (!S.first && html === S.lastHtml) return;          // nothing changed: leave the DOM (and scroll position) alone
    S.lastHtml = html;
    var view = $id('view'), tpl = document.createElement('div');
    // Read scroll state BEFORE touching the DOM: reading it later forces a layout while kept panels are
    // temporarily detached, the document shrinks, and the browser clamps the page scroll upwards.
    var y = window.scrollY, keepScroll = !S.first;
    var scrolls = Array.prototype.map.call(view.querySelectorAll('.scroll'), function (s) { return s.scrollTop; });
    view.style.minHeight = view.offsetHeight + 'px';       // prevent the document from shrinking during the swap
    tpl.innerHTML = html;
